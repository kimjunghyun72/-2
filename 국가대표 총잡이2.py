...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Game("실력을 연마"))
    ...
    for i in range(len(userFlist)):
        if userFlist[i][0] == str(ctx.message.author.name):
            if len(userFlist[i]) >= 2:  # 노래가 있다면
                ...
                titlename = str(ctx.message.author.name) + "사격선수님의 즐겨찾기"
...
bot.run('ODMzMTYwNDUxNzMwNzAyMzc2.YHuTIw.xqzCE6Jtw_Mpl7W6K89Ep-lLy-E')